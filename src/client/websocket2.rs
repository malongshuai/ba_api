...
impl WS {
    /// channel: 要订阅的通道或者listenKey
    /// names：多数时候是symbols，少数情况是只包含单个带有感叹号字符串的列表，
    /// 如果两者都不为空，说明是建立连接并订阅行情数据，
    /// 如果channel不空，而names为空，说明是建立连接并订阅账户信息
    /// 如果两者都为空，说明只是建立连接，而不订阅，等待后续手动发送订阅信息
    fn make_ws_url(channel: &str, names: &[String], base_url: &str) -> String {
        match (channel.is_empty(), names.is_empty()) {
            // 无效的参数
...
impl WsClient {
    /// 新建到bi安的ws连接客户端，
    ///
    /// channel和names都为空时，表示建立空连接，等待后面手动发送订阅
    pub async fn new(channel: &str, names: Vec<String>) -> BiAnResult<Self> {
